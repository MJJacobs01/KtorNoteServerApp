            ...
            result?.let {
                when (result.status) {
                    Status.SUCCESS -> {
                        ...
                        sharedPreferences.edit().putString(Constants.KEY_LOGGED_IN_EMAIL, curEmail).apply()
                        sharedPreferences.edit().putString(Constants.KEY_PASSWORD, curPassword).apply()
